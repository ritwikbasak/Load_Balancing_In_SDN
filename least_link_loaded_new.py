...
class Controller(RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.link_statistics=[{'byte_count': 0} for _ in range(self.no_of_servers)]
        self.ipMacMap = {}
    ...
    def add_flow(self, datapath, priority, match, actions, hard_timeout=None, cookie=0):
        '''
        ...
        for stat in body:
            for i in range(1,self.no_of_servers+1):
                if 'in_port' in stat.match and stat.match['in_port'] == i:
                    ...
                    if self.index==0:
                        if i in self.index_0_inport_stat:
                            self.index_0_inport_stat.append(stat)
                        # link_stats['initial_bytes'].append(stat.byte_count)
                        # link_stats['initial_time'].append(stat.duration_sec)
                        else:
                            self.index_0_inport_stat[i]=[stat]

                    else:
                        if i in self.index_1_inport_stat:
                            self.index_1_inport_stat.append(stat)
                        
                        else:
                            self.index_1_inport_stat[i]=[stat]
...
#***********************************************************************
        if self.index_0_inport_stat == self.index_1_inport_stat:
            if self.index==0:
                for i in self.index_0_inport_stat.keys():
                    self.count0[i] = 0
                    for j in self.index_0_inport_stat[i]:
                        self.count0[i] += j.byte_count

            tp = [0 for i in range(self.no_of_servers)]
            if self.index==1:
                for i in self.index_1_inport_stat.keys():
                    self.count1[i] = 0
                    for j in self.index_1_inport_stat[i]:
                        self.count1[i] += j.byte_count

                    tp[i] = (self.count1[i] - self.count0[i]) / 5 # unit is bytes per second
